...
public:
    ComputerClub(int num_tables, std::pair<int, int> schedule, int hourly_rate)
        : num_tables_(num_tables), open_time_(schedule.first), close_time_(schedule.second), hourly_rate_(hourly_rate) {
...
std::pair<int, int> ReadInputTime(const std::string& input_line) {
    int open_time_hour, open_time_minute, close_time_hour, close_time_minute;
    char separatorColon, separatorSpace;
    std::istringstream is(input_line);
    if (!(is >> std::noskipws >> open_time_hour >> separatorColon >> open_time_minute) || separatorColon != ':') {
        throw std::runtime_error("Error at line 2: invalid time format");
    }
    if (is.get() != ' ') {
        throw std::runtime_error("Error at line 2: invalid time format");
    }
    if (!(is >> close_time_hour >> separatorColon >> close_time_minute) || separatorColon != ':') {
        throw std::runtime_error("Error at line 2: invalid time format");
    }
    if (!CheckInputTime(open_time_hour, open_time_minute) || !CheckInputTime(close_time_hour, close_time_minute)) {
        throw std::runtime_error("Error at line 2: invalid time format");
    }
    if (open_time_hour * 60 + open_time_minute > close_time_hour * 60 + close_time_minute) {
        throw std::runtime_error("Error at line 2: invalid time format");
    }

    std::pair<int, int> schedule = std::make_pair<int, int>(open_time_hour * 60 + open_time_minute, close_time_hour * 60 + close_time_minute);
    return schedule;
}
...
bool CheckEventString(const ComputerClub& club, std::string& event_string) {
    ...
    if (!(is >> std::noskipws >> hour >> colon_separate >> minute) || colon_separate != ':')
        return false;
    if (!CheckInputTime(hour, minute))
        return false;
    ...
    if (id == 2 && (!(is >> space_separate >> table_number) || space_separate != ' ' || !(table_number >= 1 && table_number <= club.num_tables_))) {
        std::cout << space_separate << "!!" << table_number << "\n";
        return false;
    }
